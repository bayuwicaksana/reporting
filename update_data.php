...
$pg_sql = "select distinct paket.pkt_id, pkt_nama, stk_nama, 
	...
	where EXTRACT(YEAR FROM pkt_tgl_buat) = date_part('year', now())
...
?>
<form action="./update_data.php?mode=sync" method="POST">
<div class="row margin-bottom">
	...
	if(!empty($mode)) 
	{
		...
		if (!$result) { 
			...
			$mySqlresult = $DB->execute("delete from datamaster where year(pkt_tgl_buat) = year(now())");
		}
